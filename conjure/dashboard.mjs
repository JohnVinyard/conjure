...
class BasicSpectrogramView {
  ...
  init(url) {
    this.element.setAttribute("src", url);
    this.element.setAttribute("type", "2D");
  ...
  static async renderURL(url, elementId) {
    const view = new BasicSpectrogramView(elementId, url);
    view.init(url);
    view.render();
    return view;
  }
...
class TensorView {
  ...
  init(url) {
    this.element.setAttribute("src", url);
    this.element.setAttribute("type", "3D");
    this.element.setAttribute("width", "500");
    this.element.setAttribute("height", "500");
  }

  static async renderURL(url, elementId, existingView = null) {
    const view = existingView || new TensorView(elementId, url);
    view.init(url);
    view.render();
    return view;
  }
...
const conjure = async (
  ...
  const contentTypeToRootElementType = {
    "application/spectrogram+octet-stream": "tensor-view",
    "application/tensor+octet-stream": "tensor-view",
